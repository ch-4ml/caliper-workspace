...
cp benchmarks/commitBenchmarkTemplate.yaml benchmarks/commitBenchmark.yaml

sed -i "s/BYTE_SIZE/$1/g" benchmarks/commitBenchmark.yaml
sed -i "s/BATCH_SIZE/$2/g" benchmarks/commitBenchmark.yaml
sed -i "s/KEY_COUNT/$3/g" benchmarks/commitBenchmark.yaml

# Run benchmark
npx caliper launch manager --caliper-workspace . --caliper-networkconfig networks/networkConfig.yaml --caliper-benchconfig benchmarks/commitBenchmark.yaml --caliper-flow-only-test --caliper-fabric-gateway-enabled

# Rename report file
mv report.html report/views/commit-report-$1-$2-$3.html
